...
def analizar_enemigo(event):
    ...
    document.getElementById("analisis-lore").innerHTML = f'<em>"{enemigo_actual.get("lore", "")}"</em>'
    ...
    for elem, valor in resistencias.items():
        if valor >= 100:
            res_html += f"<span style='color:#00ff00'>{elem.upper()}: INMUNE</span><br>"
        elif valor > 0:
            res_html += f"<span style='color:#ffff00'>{elem.upper()}: {valor}%</span><br>"
    ...
    for elem, valor in debilidades.items():
        color = "#ff0000" if valor >= 200 else "#ff8800"
        deb_html += f"<span style='color:{color}'>{elem.upper()}: {valor}%</span><br>"
    ...
    for hab in habilidades:
        ...
        hab_html += f"<strong style='color:#ffd700'>{hab['nombre']}</strong><br>"
        hab_html += f"<small>{hab['descripcion']}</small><br>"
        hab_html += f"<span style='color:#00ff41'>Efecto: {hab['efecto']}</span>"
        hab_html += f"</div>"
