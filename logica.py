...
def abrir_inventario(event):
    """🔥 MEJORADO: Inventario con botones funcionales usando event listeners"""
    ...
    if jugador.contar_items() == 0:
        ...
        for item_nombre, cantidad in jugador.inventario.items():
            ...
            if item_data:
                ...
                item_id = f"item_{contador}"
                contador += 1
                
                html += f"<div class='item-inventario-contenedor' id='cont_{item_id}'>"
                html += f"<div class='item-inventario-info' id='info_{item_id}'>"
                ...
                if tipo == "equipamiento":
                    html += f"<button class='btn-item-accion btn-equipar' id='btn_eq_{item_id}' data-item='{item_nombre}'>⚔️ Equipar</button>"
                html += f"<button class='btn-item-accion btn-vender' id='btn_sell_{item_id}' data-item='{item_nombre}'>💰 Vender</button>"
                html += f"<button class='btn-item-accion btn-info' id='btn_info_{item_id}' data-item='{item_nombre}'>ℹ️ Info</button>"
                html += f"</div>"
                html += f"</div>"
        ...
        def setup_listeners():
            contador_local = 0
            for item_nombre in list(jugador.inventario.keys()):
                item_id = f"item_{contador_local}"
                item_data = get_item(item_nombre)
                
                # Botón equipar
                if item_data and item_data.get("tipo") == "equipamiento":
                    btn_eq = document.getElementById(f"btn_eq_{item_id}")
                    if btn_eq:
                        # Crear closure para capturar item_nombre
                        def make_equipar_handler(nombre):
                            def handler(e):
                                equipar_desde_inventario_interno(nombre)
                            return handler
                        btn_eq.addEventListener("click", make_equipar_handler(item_nombre))
                
                # Botón vender
                btn_sell = document.getElementById(f"btn_sell_{item_id}")
                if btn_sell:
                    def make_vender_handler(nombre):
                        def handler(e):
                            vender_item_interno(nombre)
                        return handler
                    btn_sell.addEventListener("click", make_vender_handler(item_nombre))
                
                # Botón info
                btn_info = document.getElementById(f"btn_info_{item_id}")
                if btn_info:
                    def make_info_handler(nombre):
                        def handler(e):
                            mostrar_info_item(nombre)
                        return handler
                    btn_info.addEventListener("click", make_info_handler(item_nombre))
                
                contador_local += 1
        
        # Ejecutar después de que el HTML esté renderizado
        setTimeout(setup_listeners, 50)
    
    modal.style.display = "flex"

def equipar_desde_inventario_interno(item_nombre):
    """Handler interno para equipar items"""
    item_data = get_item(item_nombre)
    if not item_data:
    ...
    # Cerrar y reabrir inventario para actualizar
    cerrar_inventario(None)
    from js import setTimeout
    setTimeout(lambda: abrir_inventario(None), 100)

def vender_item_interno(item_nombre):
    """Handler interno para vender items"""
    item_data = get_item(item_nombre)
    if not item_data:
        actualizar_pantalla("❌ Item no encontrado")
        return
...
def guardar_partida(event=None):
    """🔥 NUEVO: Guarda la partida actual en localStorage"""
    import json
    ...
    datos_partida = {
        ...
        "mp": jugador.mp,
        "mp_max": jugador.mp_max,
        "experiencia": jugador.experiencia,
        "exp_para_subir": jugador.exp_para_subir,
        "oro": jugador.oro,
        "oro_total_ganado": jugador.oro_total_ganado,
        "fuerza": jugador.fuerza,
        "destreza": jugador.destreza,
        "aguante": jugador.aguante,
        "inventario": jugador.inventario,
        "equipo": {},
        "combates_ganados": jugador.combates_ganados,
        "enemigos_derrotados": jugador.enemigos_derrotados,
        "daño_total_causado": jugador.daño_total_causado,
        "items_equipados": jugador.items_equipados,
        "habilidades_pasivas": jugador.habilidades_pasivas,
        "habilidades_activas": jugador.habilidades_activas
    }
    
    # Guardar equipo (solo nombres de items)
    for slot, item_data in jugador.equipo.items():
        if item_data:
            datos_partida["equipo"][slot] = item_data.get("nombre")
        else:
            datos_partida["equipo"][slot] = None
    
    # Guardar en localStorage
    localStorage.setItem("pyterra_partida_guardada", json.dumps(datos_partida))
    
    actualizar_pantalla("💾 ¡Partida guardada exitosamente!")

def cargar_partida():
    """🔥 NUEVO: Carga la partida guardada desde localStorage"""
    import json
    from js import localStorage
    
    partida_str = localStorage.getItem("pyterra_partida_guardada")
    
    if not partida_str or partida_str == "null":
        return False
    
    try:
        datos = json.loads(partida_str)
        
        # Restaurar stats básicos
        jugador.nombre = datos.get("nombre", "Reencarnado")
        jugador.nivel = datos.get("nivel", 1)
        jugador.vida = datos.get("vida", 100)
        jugador.vida_max = datos.get("vida_max", 100)
        jugador.mp = datos.get("mp", 50)
        jugador.mp_max = datos.get("mp_max", 50)
        jugador.experiencia = datos.get("experiencia", 0)
        jugador.exp_para_subir = datos.get("exp_para_subir", 100)
        jugador.oro = datos.get("oro", 0)
        jugador.oro_total_ganado = datos.get("oro_total_ganado", 0)
        
        # Restaurar stats de atributos
        jugador.fuerza = datos.get("fuerza", 10)
        jugador.destreza = datos.get("destreza", 10)
        jugador.aguante = datos.get("aguante", 10)
        
        # Restaurar inventario
        jugador.inventario = datos.get("inventario", {})
        
        # Restaurar estadísticas
        jugador.combates_ganados = datos.get("combates_ganados", 0)
        jugador.enemigos_derrotados = datos.get("enemigos_derrotados", 0)
        jugador.daño_total_causado = datos.get("daño_total_causado", 0)
        jugador.items_equipados = datos.get("items_equipados", 0)
        
        # Restaurar habilidades
        jugador.habilidades_pasivas = datos.get("habilidades_pasivas", ["Percepción del Reencarnado"])
        jugador.habilidades_activas = datos.get("habilidades_activas", [])
        
        # Restaurar equipo
        equipo_guardado = datos.get("equipo", {})
        for slot in ["mano_derecha", "mano_izquierda", "torso", "cabeza", "piernas", "pies", "anillo", "cuello"]:
            item_nombre = equipo_guardado.get(slot)
            if item_nombre and item_nombre in ITEMS_DB:
                jugador.equipo[slot] = ITEMS_DB[item_nombre].copy()
                jugador.equipo[slot]["nombre"] = item_nombre
            else:
                jugador.equipo[slot] = None
        
        actualizar_pantalla(f"✅ ¡Bienvenido de vuelta, {jugador.nombre}! Tu aventura continúa...")
        return True
...
def iniciar_juego_desde_login(event):
    """🔥 NUEVO: Llamado cuando el juego inicia desde el login"""
    # Intentar cargar partida guardada
    if event.detail.get("cargar_guardado", False):
        if cargar_partida():
            actualizar_pantalla(f"🌟 Bienvenido de vuelta, {jugador.nombre}!")
        else:
            # Si no hay partida, usar nombre del login
            jugador.nombre = event.detail.get("usuario", "Reencarnado")
            actualizar_pantalla(f"🌟 Bienvenido, {jugador.nombre}. Tu aventura comienza ahora...<br><small>Presiona NUEVO COMBATE para empezar</small>")
    else:
        # Nueva partida
        jugador.nombre = event.detail.get("usuario", "Reencarnado")
        actualizar_pantalla(f"🌟 Bienvenido, {jugador.nombre}. Tu aventura comienza ahora...<br><small>Presiona NUEVO COMBATE para empezar</small>")

# Escuchar evento de inicio de juego desde JavaScript
from js import window
window.addEventListener("juego_iniciado", iniciar_juego_desde_login)

# Mensaje inicial (se sobrescribirá cuando se inicie desde login)
actualizar_pantalla("⏳ Iniciando juego...")
