...
def actualizar_pantalla(texto):
    ...
    # 🐛 BUG CORREGIDO: append=False para reemplazar en lugar de acumular
    display(texto, target="historia", append=False)
    display(str(jugador.vida), target="vida-jugador", append=False)
    display(str(enemigo_vida), target="vida-enemigo", append=False)
    # 🔥 NUEVO: Actualizar stats adicionales
    display(str(jugador.nivel), target="nivel", append=False)
    display(f"{jugador.experiencia}/{jugador.exp_para_subir}", target="exp", append=False)
    display(str(jugador.oro), target="oro", append=False)
    display(f"{len(jugador.inventario)} items", target="inventario", append=False)
...
def dar_recompensas():
    ...
    if nivel_subio:
        msg = f"🎉 ¡VICTORIA! +{oro_ganado}💰 +{exp_ganada}✨{item_msg}\n⭐ ¡SUBISTE AL NIVEL {jugador.nivel}! Vida restaurada."
    else:
        msg = f"🎉 ¡VICTORIA! +{oro_ganado}💰 +{exp_ganada}✨{item_msg}"
